...
- [x] Removed the extra browser-side Supabase status update before chat sends; `/api/chat` owns the authoritative session status write so student messages reach the backend with one fewer round trip.
...
- `/api/chat` route — takes session ID plus typed text and/or `voiceTranscription`, writes structured `[TEXT CHAT MESSAGE]`, `[VOICE OVER AUDIO TRANSCRIPTION]`, and `[ANNOTATED CODE SNIPPET]` sections immediately, hydrates only needed assignment/submission/question/transcript fields, streams OpenRouter tokens through Ora-formatted SSE as they arrive, saves the completed AI reply to DB, and emits developer telemetry headers when requested
- Chat latency remediation — the live interview no longer waits for a full OpenRouter completion before sending the first SSE chunk; if a provider/model returns an empty, malformed, fused-word, or cut-off token stream, `/api/chat` normalizes final text, falls back to one bounded non-streaming completion when needed, and can send a final `replace` frame so the UI does not get stuck or save a broken assistant question, while the report path still uses full completions for strict JSON generation
